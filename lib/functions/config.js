...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		let stckPack = (removeAccents(config.Sticker_Pack).toString() || "🔰 ECMTZ [MaaTVZ] MaTZ ⚜️")
		...
		for (let i = 0; i < multiCommand.length; i++) {
			...
			switch (cmd) {
				...
				case 'legiao':
					if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
					await kill.sendLinkWithAutoPreview(from, 'https://github.com/anonybrmous', mess.onlyRecLz())
				break
				...
				case 'criador':
					...
					await kill.reply(from, `${hostNot}🌙 - Dev: Matheus V. - MaaTVZ [Breve>Brasil]\n\n${mess.everhost()}`, id)
				break
