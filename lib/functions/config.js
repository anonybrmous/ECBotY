...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		let stckPack = (removeAccents(config.Sticker_Pack).toString() || "🔰 Hehe Boy! [https://evolutioncine.tk] MaaTVZ ⚜️")
		...
		for (let i = 0; i < multiCommand.length; i++) {
			...
			switch (cmd) {
				...
				case 'legiao':
					if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
					await kill.sendLinkWithAutoPreview(from, 'https://evolutioncine.tk', mess.onlyRecLz())
				break
				...
				case 'criador':
					...
					await kill.reply(from, `${hostNot}🌙 - Dev: Matheus G. - MaaTVZ & Maninho [https://evolutioncine.tk]\n\n${mess.everhost()}`, id)
				break
